...
function ajax_pagination_svl($atts)
{
    ...
    $allpost .= query_ajax_pagination($post_type, $cat, $posts_per_page, $paged);
    $allpost .= '</div>';
    return $allpost;
}
add_shortcode('ajax_pagination', 'ajax_pagination_svl');
function query_ajax_pagination($post_type = 'post', $cat = 19, $posts_per_page = 5, $paged = 1)
{
    $args_svl = array(
        'post_type' => $post_type,
...
function devvn_useAjaxPagination()
{
    ...
    wp_enqueue_style('ajaxp', get_stylesheet_directory_uri() . '/AjaxPagination/Ajax_pagination.css', [], '1.0.0');
