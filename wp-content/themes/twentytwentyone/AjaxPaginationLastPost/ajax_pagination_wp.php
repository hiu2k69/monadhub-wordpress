...
function ajax_pagination_lastpost_svl($atts)
{
    $atts = shortcode_atts(
        array(
            'posts_per_page' => 4,
        ...
        $atts,
        'ajax_pagination_lastpost'
    );
    $posts_per_page = intval($atts['posts_per_page']);
    $paged = intval($atts['paged']);
    $post_type = sanitize_text_field($atts['post_type']);
    $allpost  = '<div id="result_ajaxp_last">';
    $allpost .= query_ajax_pagination_lastpost($post_type, $posts_per_page, $paged);
    $allpost .= '</div>';
    return $allpost;
}
add_shortcode('ajax_pagination_lastpost', 'ajax_pagination_lastpost_svl');
function query_ajax_pagination_lastpost($post_type = 'post', $posts_per_page = 4, $paged = 1)
{
    $args_svl = array(
        'post_type' => $post_type,
    ...
    if ($q_svl->have_posts()) {
        $allpost = '<div class="row pb-paginate position-relative ajax_pagination_lastpost"  posts_per_page="' . $posts_per_page . '" post_type="' . $post_type . '">';
...
<?php
        ...
        $allpost .= paginate_lastpost_function($posts_per_page, $paged, $total_records, $total_pages);
...
function paginate_lastpost_function($item_per_page, $current_page, $total_records, $total_pages)
{
    $pagination = '';
...
add_action('wp_ajax_LoadPostPaginationLastPost', 'LoadPostPaginationLastPost_init');
add_action('wp_ajax_nopriv_LoadPostPaginationLastPost', 'LoadPostPaginationLastPost_init');
function LoadPostPaginationLastPost_init()
{
...
add_action('wp_enqueue_scripts', 'devvn_useAjaxPaginationLastPost', 1);
function devvn_useAjaxPaginationLastPost()
{
    /** Thêm js vào website */
    wp_enqueue_script('devvn-ajax-last', esc_url(trailingslashit(get_stylesheet_directory_uri()) . 'AjaxPaginationLastPost/ajax_pagination.js'), array('jquery'), '1.0', true);
    $php_array = array(
        'admin_ajax' => admin_url('admin-ajax.php')
    );
    wp_localize_script('devvn-ajax-last', 'svl_array_ajaxp_last', $php_array);
    /*Thêm css vào website*/
    wp_enqueue_style('ajaxp', get_stylesheet_directory_uri() . '/AjaxPaginationLastPost/Ajax_pagination.css', [], '1.0.0');
